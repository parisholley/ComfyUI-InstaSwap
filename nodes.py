...
INSTASWAP_MODELS_PATH = os.path.join(models_dir, "instaswap")
FACE_MODELS_PATH = os.path.join(INSTASWAP_MODELS_PATH, "faces")

if not os.path.exists(INSTASWAP_MODELS_PATH):
...
class instaswap:
    ...
    def execute(self, enabled, input_image, swap_model, detect_gender_source, detect_gender_input, source_faces_index, input_faces_index, console_log_level, face_restore_model, face_restore_visibility, codeformer_weight, facedetection, source_image=None, face_model=None):
        ...
        if not enabled:
            ...
            logger.error("Please provide source image or a face model to swap")
            return (input_image,face_model)
...
NODE_CLASS_MAPPINGS = {
    "InstaSwapFaceSwap": instaswap,
    "InstaSwapLoadFaceModel": LoadFaceModel,
    "InstaSwapSaveFaceModel": SaveFaceModel,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "InstaSwapFaceSwap": "InstaSwap Fast Face Swap 🥸",
    "InstaSwapLoadFaceModel": "Load Face Model",
    "InstaSwapSaveFaceModel": "Save Face Model",
}
